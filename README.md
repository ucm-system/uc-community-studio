...
1. 首页查看各期活动，点击 **新建活动**，或在已有活动卡片上复制上一期。新活动预填完整基础示例，便于直接修改。
...
首页和编辑器顶部均有 **嘉宾填写模板**。打开后可以预览待填写内容和一份完整示例，点击 **复制模板与示例**，即可粘贴发送给嘉宾。

模板包含主题、副标题、讨论要点、姓名、身份介绍、技术方向、日期、起止时间、地点或会议说明，以及可选的头像和二维码。示例直接使用新建活动的同一份基础数据，并明确标为示例；不会把当前活动中其他嘉宾的信息当作例子复制。

已有活动不会因默认值更新被覆盖。填写真实活动后，请修改示例日期等内容并取消“标记为示例活动”。工具只负责生成可复制文字，不会替你发送消息。
